 ...
 */
var stud_1 = new studModel({ firstName: 'Jade', lastName: 'Bing', degree:'MS', program:'Computer Science', graduationyear:'2001', email:'jbing@uncc.edu' });
var stud_2 = new studModel({ firstName: 'Jade', lastName: 'Ros', degree:'MS', program:'Computer Science' , graduationyear:'2002', email:'jros@uncc.edu'});
var stud_3 = new studModel({ firstName: 'Chandler', lastName: 'Bing', degree:'MS', program:'Computer Science' , graduationyear:'2003', email:'cbing@uncc.edu'});
var stud_4 = new studModel({ firstName: 'Rachel', lastName: 'Green', degree:'MS', program:'Computer Science' , graduationyear:'2004', email:'rgreen@uncc.edu'});
...
app.post('/studentInfo*', urlencodedParser, function (request, response) {
  ...
  if ((Object.keys(studentReqParams).length != 0)) {
    ...
    student = studentObj.student(studentReqParams.firstName, studentReqParams.lastName, studentReqParams.degree, 
              studentReqParams.program, studentReqParams.graduationyear, studentReqParams.email);

      // validate the inputs
    request.checkBody('firstName', "Enter a valid firstName.")
    .isAlpha()
    .isLength({ max: 40 });

    request.checkBody('lastName', "Enter a valid lastName.")
    .isAlpha()
    .isLength({ max: 40 });

    request.checkBody('degree', "Enter a valid degree")
    .optional({ checkFalsy: true }).isAlpha()
    .isLength({ max: 40 });

    request.checkBody('program', "Enter a valid program")
    .isAlpha()
    .optional({ checkFalsy: true })
    .isLength({ max: 40 });

    request.checkBody('graduationyear', "Enter a valid graduation year")
    .isInt()
    .optional({ checkFalsy: true })
    .isLength({ max: 4 });

    request.checkBody('email', "Enter a valid email address.")
    .optional({ checkFalsy: true })
    .isEmail();
...
var studDoc = new studModel({ firstName: studentReqParams.firstName, 
              ...
              graduationyear:studentReqParams.graduationyear,
              email:studentReqParams.email });
...
studModel.findOneAndUpdate(
    ...
    {$set : { firstName: studentReqParams.firstName, 
							...
							program:studentReqParams.program,
              graduationyear:studentReqParams.graduationyear,
              email:studentReqParams.email  }}, // document to insert when nothing was found, TBD in a better way
    ...
    console.log("request did not include any parameters");
    var path = process.cwd();
    console.log("path from where node was started" + path);
    response.sendFile(path + '/views/index.html');
  }
